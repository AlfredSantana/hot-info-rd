...
function LinkPreviewCard({ url }) {
  ...
  if (preview === undefined) {
    return <div className="source-slide source-card source-card-loading">Cargando…</div>
  }

  if (!preview || !preview.ok) {
    return (
      <a href={url} target="_blank" rel="noopener noreferrer" className="source-slide source-card source-card-fallback">
        ...
        <div>
          ...
          <span className="source-card-fallback-text">Ver publicación original</span>
        </div>
      </a>
    )
  }
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="source-slide source-card">
      {preview.image && <img src={preview.image} alt="" className="source-card-image" />}
      <div className="source-card-body">
        <span className="source-card-domain">{preview.siteName || domain}</span>
        {preview.title && <span className="source-card-title">{preview.title}</span>}
      </div>
    </a>
  )
}
    ...
    <section className="sources-display">
      ...
      <div className="sources-display-track" ref={trackRef}>
        {sources.map((s) => {
          ...
          if (s.type === 'video') {
            return (
              <div key={s.id} className="source-slide source-card source-card-video">
